---
title: "Supplemental Figure 1.1 - STP PDX"
output:
  html_document:
    df_print: paged
---
This file is meant to create supplementary figure 1.2 for the revised manuscript. 
...
STPPDXCellRes <- readRDS("~/groups/OE0585/internal/p163v/scAbsoluteWorkflow/results/scale/STP-PDX/500/predict/out.rds")
protocolData(STPPDXCellRes) <- AnnotatedDataFrame()
colnames(STPPDXCellRes) <- gsub(colnames(STPPDXCellRes), pat = "\\-[0-9]\\.filtered\\.sorted$", rep = "")
...
# STPPDXCT8Scores <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/STP_Nuclei_CT_scores_50MB_8_2.rds")

# colnames(STPPDXCloneRes)[length(colnames(STPPDXCloneRes))] <- "Normal"
...
```{r}
STPPDXCloneList <- fread("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/STP_PDX_cell_clone_assignments.csv", header=TRUE)
...
STPPDXCloneList[Clone!='Not Assigned',.N]
))
...
After examining the singlecell profiles, and the clonal profiles, we chose k=5 for the Ploidy 4 cells.

No other ploidy states had any clones.


```{r}

ploidy4ClonesBoot <- STPPDXCloneBoot$Ploidy_4$k_5[sapply(STPPDXCloneBoot$Ploidy_4$k_5, length)>2]
...
SingleCellCopynumberMat <- assayDataElement(STPPDXCellRes, "copynumber")
...
cur.boot.res <- STPPDXCloneBoot$Ploidy_4$k_5
...
cluster.boot.ct.res.chr <- lapply(names(cur.boot.res), function(cluster){
  ...
  rownames(cur.boot.cells) <- paste0(STPPDXCloneRes$chr, ":", STPPDXCloneRes$start, "-", STPPDXCloneRes
                                     $end)
...
for (clone.tmp in clones){
    ...
    melted.clone <- data.frame(STPPDXCloneRes[,c("chr", "start","end",clone.tmp), with=FALSE])
    ...
    curCloneCTScore <- t(STPPDXCTScores$CTData$CT_CellsBins[clone.tmp,, drop=FALSE])
    ...
    CTscorePoint <- melt(STPPDXCTScores$CTData$CT_CellsChrs[clone.tmp, , drop = FALSE])
