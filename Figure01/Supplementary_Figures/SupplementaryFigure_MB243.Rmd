---
title: "Supplemental Figure 1.5 - MB243"
output:
  html_document:
    df_print: paged
---
This file is meant to create supplementary figure 1.5 for the revised manuscript. 
...
First, lets plot the bulk data for this sample. We normalize it to ploidy 4, as per the majority ploidy in our single cell results. We load in the single cell results just to get the coordinates from them
(length of chromosomes, centromere bins, etc.)

```{r}
MB243CloneRes <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/MB243_clonal_cnv.rds")

sc.coords <- MB243CloneRes[,.(chr, start, end)]
...
############################################################################
##                VISUALISE THE BULK SEQUENCING DATA FOR ST1R-NUCLEI
############################################################################

# ST1R PDX
BulkTable_MB243 <-read.table(file = "results/CNMops/MB243/XI046_ST_LFS_1_control-pdx-x_blood.bins.tsv", stringsAsFactors = F, header = T)
# BulkTable_MB243[which(BulkTable_MB243[,1]=="X"),1]<-23
# BulkTable_MB243[which(BulkTable_MB243[,1]=="Y"),1]<-24


# BulkTable_MB243 <- data.table(BulkTable_MB243)

BulkTable_MB243 <- merge(sc.coords,BulkTable_MB243, by.x = c("chr", "start", "end"), by.y = c("chr", "start", "end"), all.x = T)


# Non aggregated data
# order data per chromosome (it was alphabetically ordered in the stored data)
orderedMB243<-c()
for(i in c(1:22, "X", "Y")){orderedMB243 <- rbind(orderedMB243, BulkTable_MB243[which(BulkTable_MB243[,1]==i),])}
...
BulkData <- orderedMB243[,4]/orderedMB243[,5]
# plot(2**(log2(BulkData)) * 2, pch=16)

xy<-which(orderedMB243[,1]=="X" | orderedMB243[,1]=="Y")
notxy<-which(!(orderedMB243[,1]=="X" | orderedMB243[,1]=="Y"))

# we transfor the ratio to absolute CNVs
BulkData[notxy] <- 2**(log2(BulkData[notxy])) * 2
BulkData[xy]<- 2**(log2(BulkData[xy])) * 1

# wrangle into shape and rename
bulk.data <- data.frame(bulk=BulkData[[1]], chr=orderedMB243[,1], coordinates=1:nrow(orderedMB243))
...
# plotbulk
ggsave(file=file.path(out.dir,"MB243_Bulk.png"), plot=plotbulk, width=160*4, height=25*2, dpi=300, units = "mm")

plotbulk <- plotbulk + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())

ggsave(file = file.path(out.dir,"MB243_Bulk_noaxis.png"), plot = plotbulk, width = 160 * 4, height = 25 * 2, dpi = 300, units = "mm")
...
# STPRPDXres <- readRDS("~/groups/OE0585/internal/p163v/scAbsoluteWorkflow/results/scale/MB243/500/predict/out.rds")
SampleCellRes <- readRDS("~/groups/OE0585/internal/p163v/scAbsoluteWorkflow/results/scale/MB243-Nuclei/500/predict/out.rds")
...
SampleNucCloneBoot <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/mb243_boot_res_ploidyfixed.rds")

SampleCloneRes <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/MB243_draft3_clonal_cnv.rds")

SampleCTScores <- readRDS(paste0("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/MB243_CT_scores_50MB_",ct.threshold,"_2.rds"))
...
```{r}
SampleCloneList <- fread("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/MB243_cell_clone_assignments.csv", header=TRUE)
...
ploidy3ClonesBoot <- SampleNucCloneBoot$Ploidy_3$k_1[sapply(SampleNucCloneBoot$Ploidy_3$k_1, length)>2]

temp <- lapply(ploidy3ClonesBoot, `[[`, "cells")

cellCloneDT <- data.table(data.frame(Cell=unlist(temp), Clone=rep(names(temp), times=sapply(temp, length))))

cellCloneDT[,Clone:=paste0("Clone_3_", .GRP), Clone]


ploidy4ClonesBoot <- SampleNucCloneBoot$Ploidy_4$k_8[sapply(SampleNucCloneBoot$Ploidy_4$k_8, length)>2]
temp2 <- lapply(ploidy4ClonesBoot, `[[`, "cells")

cellCloneDT2 <- data.table(data.frame(Cell=unlist(temp2), Clone=rep(names(temp2), times=sapply(temp2, length))))
cellCloneDT2[,Clone:=paste0("Clone_4_", .GRP), Clone]
...
cur.boot.res <- c(SampleNucCloneBoot$Ploidy_3$k_1, SampleNucCloneBoot$Ploidy_4$k_8)
