---
title: "Supplemental Figure 1.1 - ST1R PDX"
output:
  html_document:
    df_print: paged
---
This file is meant to create supplementary figure 1.4 for the revised manuscript. 
...
First, lets plot the bulk data for this sample. We normalize it to ploidy 2, as per single cell results. We load in the single cell results just to get the coordinates from them
...
sc.coords <- ST1RPDXCloneRes[,.(chr, start, end)]
...
# ST1R PDX
BulkTable_ST1RPDX <-read.table(file = "/b06x-isilon/b06x-c/chromothripsis/results/external/XI046_ST_LFS_1/CNV/CNMops/control-pdx-x/XI046_ST_LFS_1_control-pdx-x_blood.bins.tsv", stringsAsFactors = F, header = T)
# BulkTable_ST1RPDX[which(BulkTable_ST1RPDX[,1]=="X"),1]<-23
# BulkTable_ST1RPDX[which(BulkTable_ST1RPDX[,1]=="Y"),1]<-24


# BulkTable_ST1RPDX <- data.table(BulkTable_ST1RPDX)

BulkTable_ST1RPDX <- merge(sc.coords,BulkTable_ST1RPDX, by.x = c("chr", "start", "end"), by.y = c("chr", "start", "end"), all.x = T)


# Non aggregated data
# order data per chromosome (it was alphabetically ordered in the stored data)
orderedST1RPDX<-c()
for(i in c(1:22, "X", "Y")){orderedST1RPDX <- rbind(orderedST1RPDX, BulkTable_ST1RPDX[which(BulkTable_ST1RPDX[,1]==i),])}
...
BulkData <- orderedST1RPDX[,4]/orderedST1RPDX[,5]
# plot(2**(log2(BulkData)) * 2, pch=16)

xy<-which(orderedST1RPDX[,1]=="X" | orderedST1RPDX[,1]=="Y")
notxy<-which(!(orderedST1RPDX[,1]=="X" | orderedST1RPDX[,1]=="Y"))
...
bulk.data <- data.frame(bulk=BulkData[[1]], chr=orderedST1RPDX[,1], coordinates=1:nrow(orderedST1RPDX))
...
# plotbulk
ggsave(file=file.path(out.dir,"ST1R_PDX_Bulk.png"), plot=plotbulk, width=160*4, height=25*2, dpi=300, units = "mm")

plotbulk <- plotbulk + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())

ggsave(file = file.path(out.dir,"ST1R_PDX_Bulk_noaxis.png"), plot = plotbulk, width = 160 * 4, height = 25 * 2, dpi = 300, units = "mm")
...
SampleNucCloneBoot <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/st1r_pdx_boot_res.rds")

SampleCloneRes <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1RPDX_clonal_cnv.rds")

SampleCTScores <- readRDS(paste0("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1R_PDX_CT_scores_50MB_",ct.threshold,"_2.rds"))
...
```{r}
SampleCloneList <- fread("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1R_PDX_cell_clone_assignments.csv", header=TRUE)
...
ploidy2ClonesBoot <- SampleNucCloneBoot$Ploidy_2$k_3[sapply(SampleNucCloneBoot$Ploidy_2$k_3, length)>2]

temp <- sapply(ploidy2ClonesBoot, `[[`, "cells")
...
cur.boot.res <- SampleNucCloneBoot$Ploidy_2$k_3
...
for (clone.tmp in clones){
    ...
    ############################################################################
    ##      PLOT CHR8 for this clone
    ############################################################################
    


    p.all.cells2.b <- (ggplot(melted[melted$chrXY=="8",]) + 
      geom_tile(aes(x=reorder(Chromosome,order), 
                                       y=reorder(Cells, order), fill=factor(value))) +
      scale_fill_manual(name = "Copy Number", values = pcol)) +
      theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(), 
    ...
    fig.file <- paste0(out.dir,'/', sample.tmp, '_', clone.tmp, '_chr8.heatmap.withchrlabels.pdf')
    png(file = fig.file, width = 45, height = (length(clone.cells)/length(unique(cellCloneDT$Cell))*35), unit="in", res=600)
    print(p.all.cells2.b)
    dev.off()
    ...
    # if(clone.tmp != "Clone_1"){
      p.all.cells2.b <- p.all.cells2.b + theme(strip.text.x = element_blank(), strip.text.y = element_blank())
    # }
    fig.file <- paste0(out.dir, "/", sample.tmp, "_", clone.tmp, "_chr8.heatmap.png")
    png(file = fig.file, width = 45, height = (length(clone.cells)/length(unique(cellCloneDT$Cell))*35), unit="in", res=600)
    print(p.all.cells2.b)
    dev.off()
    ...
    fig.file <- paste0(out.dir, "/", sample.tmp, "_", clone.tmp, "_chr8.nolegend.png")
    png(file = fig.file, width = 45, height = (length(clone.cells)/length(unique(cellCloneDT$Cell))*35), unit="in", res=600)
    print(p.all.cells2.b)
    dev.off()
    ...
    p.all.cells2.b<- (ggplot(melted.clone[melted.clone$chrXY=="8",]) + 
      geom_tile(aes(x=reorder(Chromosome,order), 
                                       y=reorder(Cells, order), fill=factor(value))) +
      scale_fill_manual(name = "Copy Number", values = pcol)) +
      theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(), 
    ...
    fig.file <- paste0(out.dir,'/', sample.tmp, '_', clone.tmp, '_clone_pseudobulk_chr8.heatmap.png')
    png(file = fig.file, width = 45, height = 2.5, unit="in", res=600)
    print(p.all.cells2.b)
    dev.off()
    ...
    fig.file <- paste0(out.dir, "/", sample.tmp, "_", clone.tmp, "_clone_pseudobulk_chr8.heatmap.nolegend.png")
    png(file = fig.file, width = 45, height = 2.5, unit="in", res=600)
    print(p.all.cells2.b)
    dev.off()
