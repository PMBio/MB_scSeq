---
title: "Supplemental Figure 1.1 - ST1R Nuclei"
output:
  html_document:
    df_print: paged
---
This file is meant to create supplementary figure 1.3 for the revised manuscript. 
...
sc.coords <- ST1RNucCloneRes[,.(chr, start, end)]
...
# ST1R
BulkTable_ST1R <-read.table(file = "/omics/groups/OE0014/internal/share/B060_Stuttgart_case/cnv_CNmops/ST_LFS_1/XI046_ST_LFS_1_relapse01/XI046_ST_LFS_1_relapse01_control.bins.tsv", stringsAsFactors = F, header = T)
# BulkTable_ST1R[which(BulkTable_ST1R[,1]=="X"),1]<-23
# BulkTable_ST1R[which(BulkTable_ST1R[,1]=="Y"),1]<-24


# BulkTable_ST1R <- data.table(BulkTable_ST1R)

BulkTable_ST1R <- merge(sc.coords,BulkTable_ST1R, by.x = c("chr", "start", "end"), by.y = c("chr", "start", "end"), all.x = T)


# Non aggregated data
# order data per chromosome (it was alphabetically ordered in the stored data)
orderedST1R<-c()
for(i in c(1:22, "X", "Y")){orderedST1R <- rbind(orderedST1R, BulkTable_ST1R[which(BulkTable_ST1R[,1]==i),])}
...
BulkData <- orderedST1R[,4]/orderedST1R[,5]
plot(2**(log2(BulkData)) * 2, pch=16)

xy<-which(orderedST1R[,1]=="X" | orderedST1R[,1]=="Y")
notxy<-which(!(orderedST1R[,1]=="X" | orderedST1R[,1]=="Y"))
...
bulk.data <- data.frame(bulk=BulkData[[1]], chr=orderedST1R[,1], coordinates=1:nrow(orderedST1R))
...
# plotbulk
ggsave(file=file.path(out.dir,"ST1R_Nuclei_Bulk.png"), plot=plotbulk, width=160*4, height=25*2, dpi=300, units = "mm")

plotbulk <- plotbulk + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank())

ggsave(file = file.path(out.dir,"ST1R_Nuclei_Bulk_noaxis.png"), plot = plotbulk, width = 160 * 4, height = 25 * 2, dpi = 300, units = "mm")
...
SampleCellRes <- readRDS("~/groups/OE0585/internal/p163v/scAbsoluteWorkflow/results/scale/STPR-Nuclei/500/predict/out.rds")
...
SampleNucCloneBoot <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/st1r_nuc_boot_res.rds")

SampleCloneRes <- readRDS("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1R_clonal_cnv.rds")

SampleCTScores <- readRDS(paste0("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1R_Nuclei_CT_scores_50MB_",ct.threshold,"_2.rds"))
...
colnames(SampleCloneRes)[length(colnames(SampleCloneRes))] <- "Diploid"
...
```{r}
SampleCloneList <- fread("/omics/groups/OE0540/internal/projects/chromothripsis_medulloblastoma/scAbsolute_clones/ST1R_Nuc_cell_clone_assignments.csv", header=TRUE)
...
ploidy2ClonesBoot <- SampleNucCloneBoot$Ploidy_2$k_6[sapply(SampleNucCloneBoot$Ploidy_2$k_6, length)>2]

names(ploidy2ClonesBoot) <- c('Clone_1', 'Diploid')
...
cur.boot.res <- SampleNucCloneBoot$Ploidy_2$k_6
...
for (clone.tmp in clones){
    ...
    CTscorePoint <- reshape2::melt(SampleCTScores$CTData$CT_CellsChrs[clone.tmp, , drop = FALSE])
    colnames(CTscorePoint) <- c("Clone","chrXY", "CT")
    CTlocs <- merge(CTlocs, CTscorePoint, by="chrXY")


    resample <- data.table(reshape2::melt(cluster.boot.ct.res.chr[[clone.tmp]]))
