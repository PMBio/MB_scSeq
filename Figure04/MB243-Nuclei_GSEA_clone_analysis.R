...
o.dir <- "/omics/groups/OE0540/internal/projects/przybilm/medulloblastoma/scRNA_analysis"
...
marker.files <- list.files(paste0("/omics/groups/OE0540/internal/projects/przybilm/medulloblastoma/scRNA_analysis/scanpy"), pattern="scDNA_clones_only", full.names=T, recursive = T)
marker.files <- marker.files[grep(sample.tmp, marker.files)]
...
processed.marker.files <- list(clone1.file, clone2.file, clone3.file, clone4.file, clone5.file)
marker.files <- list()
final.marker.files <- list()
final.clone.ids <- c()

# subclones present
subclone.id <- c("clone1", "clone2", "clone3", "clone4", "clone5")
...
allm <- allm[,c("clone1", "clone2", "clone3", "clone4", "clone5")]
...
ht1 <- Heatmap(allm, 
               ...
               top_annotation = HeatmapAnnotation(column_barplot = anno_barplot(marker.matrix, 
                                                                                ...
                                                                                axis_param = list(at = c(0, 250, 500, 750, 1000, 1250, 1500),
...
# add a bulk heatmap
bulk.gsea.results <- read.table(paste0(o.dir, "/Hallmark_GSEA_bulkRNA.txt"), header = T)
...
bulk.gsea.results.wide.mat <- bulk.gsea.results.wide.mat[rownames(allm),]

ht2 <- Heatmap(bulk.gsea.results.wide.mat, 
               name = "bulk", 
               col = circlize::colorRamp2(colors, my_palette), 
               cluster_rows = F, 
               cluster_columns = FALSE,
...
pdf(paste0(sample.tmp, "/DEG/Combined_GSEA_bulk_MB243.pdf"),width=6,height = 5,pointsize=0.1)
print(ht1 + ht2)
dev.off()
