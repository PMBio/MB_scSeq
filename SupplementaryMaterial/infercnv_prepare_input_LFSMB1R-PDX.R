...
sample.id <- "LFSMB1R-PDX"

# make list of working directories
w.dir <- file.path(o.dir, paste0(sample.id, "_mouse"))
...
## LFSMB1R-NUCLEI
c.dir <- "/icgc/dkfzlsdf/analysis/B260/projects/chromothripsis_medulloblastoma/data/10XRNA5P/LFSMB1R/LFSMB1R-PDX/outs/"
...
# cell.clusters <- read.csv("/icgc/dkfzlsdf/analysis/B260/projects/przybilm/medulloblastoma/scRNA_analysis/scanpy/STP-PDX_metadata.csv")
# 
# # get endothelial cells
# cell.clusters_WT <- as.character(cell.clusters[grep("miroglia|astrocytes", cell.clusters$new_clusters), "X"])
# cell.clusters_WT <- str_split_fixed(cell.clusters_WT , "-1", 2)[,1]

# read in the mouse cells
mouse.cells <- read.table('/icgc/dkfzlsdf/analysis/B260/projects/chromothripsis_medulloblastoma/data/10XRNA5P/LFSMB1R/mm10_and_hg19/sample4.mapping.counts.txt', 
                          header = T, stringsAsFactors = F)
mouse.cells <- mouse.cells[mouse.cells$ratio < 1,]
mouse.cells <- str_split_fixed(mouse.cells$cell , "-1", 2)[,1]

# get normal cells
# normal.cells <- cell.clusters_WT
normal.cells <- mouse.cells

# read in genes x cells matrix from 10x genomics for STP-PDX with microglia reference
# matrix.tmp.neg <- Read10X(paste0("/icgc/dkfzlsdf/analysis/B260/projects/chromothripsis_medulloblastoma/data/10XRNA5P/STP/STP-PDX/outs/filtered_feature_bc_matrix"))
matrix.tmp.neg <- matrix.tmp
# colnames(matrix.tmp.neg) <- str_split_fixed(colnames(matrix.tmp.neg), "-1", 2)[,1]
mat.neg.list <- list(matrix.tmp.neg[, colnames(matrix.tmp.neg) %in% normal.cells])

# read in the barcodes for the normal cells if using a reference
cell.clusters <- read.csv("/icgc/dkfzlsdf/analysis/B260/projects/przybilm/medulloblastoma/scRNA_analysis/scanpy/LFSMB1R-PDX_metadata.csv")

# get the malignant cells
cell.clusters_malignant <- str_split_fixed(cell.clusters$X , "-1", 2)[,1]
mat.pos.list <- list(matrix.tmp[,cell.clusters_malignant])
