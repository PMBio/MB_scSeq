...
marker.files <- list.files(paste0("/icgc/dkfzlsdf/analysis/B260/projects/przybilm/medulloblastoma/scRNA_analysis/scanpy"), pattern="scDNA_clones_only", full.names=T)
...
processed.marker.files <- list(clone2.file, clone3.file, clone4.file, clone5.file, clone6.file)
marker.files <- list()
final.marker.files <- list()
final.clone.ids <- c()

# subclones present
subclone.id <- c("clone2", "clone3", "clone4", "clone5", "clone6")
...
allm <- allm[,c("clone2", "clone3", "clone4", "clone5", "clone6")]
...
ggplot(complete.melt, aes(fill = Clones, y=proportion, x=chr)) + 
  ...
  theme(axis.text = element_text(colour = "black", size = 12, face = "bold" ),
        ...
        legend.position = "top")
ggsave(paste0(sample.tmp, "/DEG/barplot_DEG_clone_chromosome.pdf"), width = 12, height = 5)
