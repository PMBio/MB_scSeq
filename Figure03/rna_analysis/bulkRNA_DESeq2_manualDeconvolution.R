...
list.of.packages <- c("reshape2", "optparse", "MuSiC", "RColorBrewer", "ggplot2", "biomaRt", "httr", "data.table", "Seurat", "SeuratDisk",
...
# results <- results[results$log2FoldChange > 1 | results$log2FoldChange < -1,]
# results <- results[results$padj < 0.05,]
# results <- results[results$pvalue <= 0.00001,]
# results <- results[results$pvalue <= 0.00001 & results$padj < 0.05,]
# write.table(results, "//omics/groups/OE0540/internal/projects/przybilm/medulloblastoma/MB_CT_vs_NCT_DESeq2_TCC_GROUP_filtered.txt", col.names = T, row.names = F, quote = F, sep = "\t")
...
pdf("/omics/groups/OE0540/internal/projects/przybilm/medulloblastoma/revision/scrna_analysis/bulkRNA_FF_without_immune_barplot_PCA.pdf")
barplot(100*p.variance.explained, las=2, xlab='', ylab='% Variance Explained')
dev.off()

loadings$ens_id <- str_split_fixed(rownames(loadings), "\\.", 2)[,1]

# merge the results with the gene names
loadings <- merge(loadings, diff.genes, by = "ens_id")
write.table(loadings, "/omics/groups/OE0540/internal/projects/przybilm/medulloblastoma/revision/scrna_analysis/MB_CT_vs_NCT_DESeq2_PCA_component.txt", col.names = T, row.names = F, quote = F, sep = "\t")
